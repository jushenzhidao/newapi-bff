...
def _render_import_doc(api_key: str, bff_origin: str) -> str:
    ...
    return (
        ...
        "4. **先定位本机 WorkBuddy 模型配置文件的真实路径**（不同系统/版本位置可能不同，"
        "不要假设固定路径，先探测再读写）：\n"
        "   - Windows（10/11 通用）：在 PowerShell 执行 "
        "`Test-Path \"$env:USERPROFILE\\.workbuddy\\models.json\"`；"
        "存在则路径即 `$env:USERPROFILE\\.workbuddy\\models.json`。\n"
        "   - macOS / Linux：执行 `ls ~/.workbuddy/models.json`；"
        "存在则路径即 `~/.workbuddy/models.json`。\n"
        "   - 若两个默认位置都探测不到 → 用对应系统的默认路径**新建**空文件 "
        "（Windows: `%USERPROFILE%\\.workbuddy\\models.json`；macOS/Linux: `~/.workbuddy/models.json`），"
        "内容初始化为 `[]`。\n"
        "   定位到文件后读取它，并判断它的「根形态」：\n"
        "   - 文件不存在（刚新建的 `[]`） → 形态 = **顶层数组**。\n"
